...
- `MetaGraphTransport` sends one request to a configured HTTPS Graph API URL. Production accepts only the default HTTPS `graph.facebook.com` origin, rejects non-default ports and redirects, uses an injected `AbortSignal.timeout` boundary, and never retries automatically; custom origins require explicit test mode and are not a production transport override.
